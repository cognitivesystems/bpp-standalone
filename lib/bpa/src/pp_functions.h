...
public:
  ...
  /*
   * To get the correct Palletization order of boxes not marked as "packed" in a bin configuration (and setting them to
   * "packed")
   ...
   * \return A bool: True: All blocking constellations could be eliminated; False: still blocking constellations are
   * existing
   */
  bool sortBoxesRemoveBlock(std::vector<Box>& packing_plan, Bin& abin);

  /// paritally ordered set: consider the boxes orders according to tool change times
  ...
  double CLEARANCE;  // 50cm;  The minimum space between the Bin height and the ground surface of the gripper at its
                     // highest position
