...
Eigen::Vector3d PhysicsBullet::castRays(Eigen::Vector3d& point, Eigen::Vector3d& direction)
{
    ...
    if (m_dynamicsWorld)
    {
        ...
        std::shared_ptr<btRigidBody> pointTemp(localCreatePointSphere(btVector3((float)(point(0) - 0.01 * direction(0)),
                                                                                (float)(point(1) - 0.01 * direction(1)),
...
double PhysicsBullet::getSupportArea(bpa::Box& new_box, bpa::Box& old_box)
{
    ...
    new_box.position.position[2]=0.9;
    if (this->isBoxContact(new_box, old_box, resultCallback))
    {
        std::cout << "Found Contact !!!!!!!!!!!!!!!!!!! --> " << resultCallback.collisionVec.size() <<  std::endl;

        for (size_t i = 1; i < resultCallback.collisionVec.size(); ++i)
        {
            // get the down side contact area
            double dist = (resultCallback.posWorldOnAVec[i].getX() - resultCallback.posWorldOnAVec[0].getX()) *
                    (resultCallback.posWorldOnAVec[i].getX() - resultCallback.posWorldOnAVec[0].getX()) +
                    (resultCallback.posWorldOnAVec[i].getY() - resultCallback.posWorldOnAVec[0].getY()) *
                    (resultCallback.posWorldOnAVec[i].getY() - resultCallback.posWorldOnAVec[0].getY());
...
double PhysicsBullet::getContactArea(bpa::Box& new_box, bpa::Box& old_box)
{
    ...
    if (this->isBoxContact(new_box, old_box, resultCallback))
    {
        for (size_t i = 1; i < resultCallback.collisionVec.size(); ++i)
        {
            ...
            else */ if (floatEqual(resultCallback.posWorldOnAVec[1].getX(), resultCallback.posWorldOnAVec[0].getX()) &&
                        floatEqual(resultCallback.posWorldOnAVec[2].getX(), resultCallback.posWorldOnAVec[0].getX()) &&
                        floatEqual(resultCallback.posWorldOnAVec[3].getX(), resultCallback.posWorldOnAVec[0].getX()))
            {
                dist = (resultCallback.posWorldOnAVec[i].getZ() - resultCallback.posWorldOnAVec[0].getZ()) *
                        (resultCallback.posWorldOnAVec[i].getZ() - resultCallback.posWorldOnAVec[0].getZ()) +
                        (resultCallback.posWorldOnAVec[i].getY() - resultCallback.posWorldOnAVec[0].getY()) *
                        (resultCallback.posWorldOnAVec[i].getY() - resultCallback.posWorldOnAVec[0].getY());
            }
            else
            {
                dist = (resultCallback.posWorldOnAVec[i].getX() - resultCallback.posWorldOnAVec[0].getX()) *
                        (resultCallback.posWorldOnAVec[i].getX() - resultCallback.posWorldOnAVec[0].getX()) +
                        (resultCallback.posWorldOnAVec[i].getZ() - resultCallback.posWorldOnAVec[0].getZ()) *
                        (resultCallback.posWorldOnAVec[i].getZ() - resultCallback.posWorldOnAVec[0].getZ());
            }
...
double PhysicsBullet::getMinimumDistance(bpa::Box& new_box)
{
    ...
    btVector3 origin(new_box.position.position(0) + new_box.center_of_mass.position(0),
                     new_box.position.position(1) + new_box.center_of_mass.position(1),
                     new_box.position.position(2) + new_box.center_of_mass.position(2));
    std::shared_ptr<btConvexShape> colShape(
                new btBoxShape(btVector3(size.getX() / 2.0, size.getY() / 2.0, size.getZ() / 2.0)));
...
double PhysicsBullet::getMinimumDistance(bpa::Box& new_box, bpa::Box& old_box)
{
    ...
    btVector3 origin_new(new_box.position.position(0) + new_box.center_of_mass.position(0),
                         new_box.position.position(1) + new_box.center_of_mass.position(1),
                         new_box.position.position(2) + new_box.center_of_mass.position(2));
    std::shared_ptr<btConvexShape> col_shape_new(
                new btBoxShape(btVector3(size_new.getX() / 2.0, size_new.getY() / 2.0, size_new.getZ() / 2.0)));
    btTransform new_box_transform;
    new_box_transform.setIdentity();
    ...
    std::shared_ptr<btConvexShape> col_shape_old(
                new btBoxShape(btVector3(size_old.getX() / 2.0, size_old.getY() / 2.0, size_old.getZ() / 2.0)));
...
btRigidBody* PhysicsBullet::localCreateRigidBodyRot(btScalar mass, btCollisionShape* shape, btVector3 origin,
                                                    btQuaternion q)
{
    btAssert((!shape || shape->getShapeType() != INVALID_SHAPE_PROXYTYPE));
    shape->setMargin(0.0);
...
bool PhysicsBullet::isPointContact(Eigen::Vector3d& point)
{
    ...
    if (resultCallback.collisionVec.size() > 0)
    {
        return true;
    }
    else
...
bool PhysicsBullet::isBoxContact(bpa::Box new_box, bpa::Box& old_box, ContactResultCallback& resultCallback)
{
    ...
    if (m_dynamicsWorld)
    {
        std::cout << "++++++++++++++++++++++++++++++ Checking Dynamics World ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
        btScalar mass(0.0);
        btVector3 size(new_box.m_length, new_box.m_width, new_box.m_height);
        btVector3 origin(new_box.position.position(0) + new_box.center_of_mass.position(0),
                         new_box.position.position(1) + new_box.center_of_mass.position(1),
                         new_box.position.position(2) + new_box.center_of_mass.position(2));
        std::shared_ptr<btCollisionShape> colShape(
                    new btBoxShape(btVector3(size.getX() / 2.0, size.getY() / 2.0, size.getZ() / 2.0)));
        ...
        std::shared_ptr<btCollisionShape> shapeOld(
...
#pragma omp critical
        ...
        std::cout << "collision vec size -----------------------------> " << resultCallback.collisionVec.size() << std::endl;
        for (size_t i = 0; i < resultCallback.collisionVec.size(); ++i)
        {  // distance are negtive
            std::cout << "resultCallback.distanceVec[i] ---> " << resultCallback.distanceVec[i] << std::endl;
        ...
        if ((resultCallback.collisionVec.size() > 0) && (floatLessEqual(std::fabs(minDistance), 0.001)))
        {
            std::cout << "Setttting true --------------------------------> " << resultCallback.collisionVec.size() << "    " <<  minDistance << std::endl;

            result = true;
        }
        else
        {
            std::cout << "Setttting false --------------------------------> " << resultCallback.collisionVec.size() << "    " <<  minDistance << std::endl;
...
#pragma omp critical
        ...
        std::cout << "Collision points ----> " << resultCallback.collisionVec.size() << std::endl;

        for (size_t i = 0; i < resultCallback.collisionVec.size(); ++i)
        {  // distance are negtive

            std::cout << "Collision distance  ----> " << resultCallback.distanceVec[i] << std::endl;

            if (floatLessThan(resultCallback.distanceVec[i], maxDistance))
            {
                maxDistance =
                        resultCallback.distanceVec[i]; /*std::cout << "collide distance is = " << maxDistance << std::endl;*/
...
btScalar PhysicsBullet::ContactResultCallback::addSingleResult(btManifoldPoint& cp,
