...
Bin BinPackingPlanner::solveWithOneFunction(Bin& abin, std::vector<Box> boxes_on_holdingplatform)
{
    ...
    //    {
    //        std::cout << "HA UUID " <<b.m_id <<" "<< b.m_name << "  l " << b.m_length << "  w " << b.m_width << "  h "
    //        << b.m_height << "  mass " << b.m_mass <<" x " << b.position.position(0) << " y " << b.position.position(1)
    //        << " z " << b.position.position(2) << "  rotated " << b.is_rotated << "  rotation " << b.rotation<< " " <<
    //        b.material << ", " << b.box_labels[0] << std::endl;
    //    }
    ...
    while (addNexBoxToPackingConfigurationOneFunction(abin, holding, &score) && iter < 10000)
    {
        ...
        for (bpa::FittingPoint fp:abin.fitting_points){
            std::cout << "Fitting point " << fp.coordinates.position.transpose() << std::endl;
        }
        std::cout << "==========================================" << std::endl;
            ...
            if (position_iterator->score >= 0)
            {
                for (int i = 0; i < paramsPtr_->search_width() && position_iterator != mixed_scores.end(); ++i)
                {
                    if (position_iterator->score >= 0)
                    {
                        ...
                        curr_testing.addNewBox(position_iterator->box_to_pack, position_iterator->point_to_pack,
                        ...
                        if (curr_cum_score > curr_max_cum_score)
                        {
                            ...
                            curr_best_solution.addNewBox(position_iterator->box_to_pack, position_iterator->point_to_pack,
                                                         holding_curr_best_solution, position_iterator->temp_a_helt);
                                                       ...
                                                       std::vector<Box> boxes_on_holdingplatform)
{
    ...
    while (!solved && iter < 10000)
    {
        ...
        else
        {
            ...
            std::sort(bin_before_testing.fitting_points.begin(), bin_before_testing.fitting_points.end(),
                      compareFittingpointScores);
                ...
                for (int i = 0; i < paramsPtr_->search_width() && fp_scores != bin_before_testing.fitting_points.end(); ++i)
                {
                    if (fp_scores->score >= 0)
                    {
                        ...
                        if (curr_cum_score > curr_max_cum_score)
                        {
                            ...
                            curr_best_solution.copyData(bin_before_testing);
                            holding_curr_best_solution.copyData(holding_before_testing);
                            curr_best_solution.addNewBox(box_to_pack, point_to_pack, holding_curr_best_solution,
                                                         point_to_pack.temp_a_helt);
                                    ...
                                    double& distance_y)
{
    ...
    case 4:
        ...
        break;
    default:
        cout << "error in givePosition()" << endl;
    }
            ...
            b.position.position(2) + b.m_height > fp.coordinates.position(2))
    {
        ...
        case 4:
            ...
            break;
        default:
            cout << "error in givePosition MARK:A" << endl;
        }
    }
...
double BinPackingPlanner::giveScore(Box& abox, FittingPoint& fp, Bin& abin, double& helt)
{
    ...
    std::cout << "Fitting Point ---> " << fp.coordinates.position.transpose() << std::endl;
    std::cout << "Box ---> " << abox.m_name << " " << abox.is_rotated << " " << abox.is_simulated << std::endl;
    ...
    {
        std::cout << "+++++++++++++++++++ Check failed -1" << std::endl;
    ...
    if (abox.is_dangerous)
    {
        ...
        {
            std::cout << "+++++++++++++++++++ Check failed 0" << std::endl;
    ...
    //    }
    /////////////////////////////////////////////////////////////////material
    //    if( (abox.material == "drum" || abox.material == "Plastic") && (fp.coordinates.position(2) + abox.m_height) <
    //    2.8) {
    //        return -1;
    //    }
    //    if(abox.material == "styrofoam" && floatLessEqual(abox.m_mass, 50) && (fp.coordinates.position(2) +
    //    abox.m_height) < 2.0) {
    //        return -1;
    //    }
    ...
    {
        if ((abox.material == "styrofoam" || abox.material == "Styrofoam") && abox.tool_name == "foamgripper" &&
                (!abox.is_rotated))
        {
            std::cout << "+++++++++++++++++++ Check failed 1" << std::endl;
            return -1;
    ...
    if ((abox.material == "styrofoam" || abox.material == "Styrofoam") && abox.tool_name == "cranepalletfork" &&
            (!abox.is_rotated))
    {
        std::cout << "+++++++++++++++++++ Check failed 2" << std::endl;
    ...
    if ((abox.is_stackable == false) && (abin.bin_height - (fp.coordinates.position(2) + abox.m_height)) > 0.4)
    {
        ...
        std::cout << "+++++++++++++++++++ Check failed 3" << std::endl;
    ...
    {
        std::cout << "+++++++++++++++++++ Check failed 4" << std::endl;
    ...
    {
        contact_area += abox.m_width * abox.m_height;
    }  // on the bin edges
    if (floatEqual(u_y, 0) || floatEqual(u_y + abox.m_width, abin.bin_width))
    {
    ...
    //    if( (abox.m_length < 0.35) && (abox.m_width < 0.35) && (fp.coordinates.position(2) + abox.m_height) < 2.6) {
    //    return -1;}
...
bool BinPackingPlanner::addNextBoxToPackingConfigurationTwoFunctions(Bin& abin, HoldingPlatform& holding,
                                                                     double* cum_score)
{
    // Initializing some Variables
    Box box_to_pack(0, 0, 0, 0, "", std::vector<std::string>());
...
bool BinPackingPlanner::addNexBoxToPackingConfigurationOneFunction(Bin& abin, HoldingPlatform& holding,
                                                                   double* cum_score)
{
    // Initializing some Variables
    Box box_to_pack(0, 0, 0, 0, "", std::vector<std::string>());
...
//#pragma omp parallel for collapse(2) shared(scores, helts)
        for (size_t holding_box_id = 0; holding_box_id < holding.boxes_to_pack.size(); ++holding_box_id)
        {
            for (size_t fitting_point_id = 0; fitting_point_id < abin.fitting_points.size(); ++fitting_point_id)
            {
                ...
                scores[holding_box_id * abin.fitting_points.size() + fitting_point_id] =
                        giveMixedScore(holding.boxes_to_pack[holding_box_id], fp, abin /*,holding*/, helt_output);
                helts[holding_box_id * abin.fitting_points.size() + fitting_point_id] = helt_output;

                std::cout << "Score --> " << scores[holding_box_id * abin.fitting_points.size() + fitting_point_id]  << std::endl;
                std::cout << "Helt --> " << helts[holding_box_id * abin.fitting_points.size() + fitting_point_id]  << std::endl;
...
#ifdef DEBUG_PRINT
            std::cout << "Start ---------------------------ID " << box_to_pack.m_id << " " << box_to_pack.m_name
                      << "  L= " << box_to_pack.m_length << "  W=" << box_to_pack.m_width << "  H=" << box_to_pack.m_height
                      << "  mass= " << box_to_pack.m_mass << " at position " << point_to_pack.coordinates.position.transpose()
                      << " rotated=" << box_to_pack.is_rotated << std::endl;
