...
namespace bpa
{
HoldingPlatform::HoldingPlatform(double bin_l, double bin_w, bool GENERATE_SIMULATED_BOXES,
                                 std::vector<Box> boxes_on_holdingplatform, int startid)
{
...
void HoldingPlatform::generateRotationBoxes()
{
  ...
  // - which simply means if one of the boxes (the original or the rotated one) will be added to the bin, then the other
  // will
...
void HoldingPlatform::generateSimulatedBoxes()
{
  ...
  for (Box abox : boxes_to_pack)
  {
    for (Box bbox : boxes_to_pack)
    {
      ...
      if (!isSimulatedBox(abox.m_id) && !isSimulatedBox(bbox.m_id) &&
          abox.box_labels.size() == bbox.box_labels.size())  // why???
      {
        if (floatEqual(abox.m_length, bbox.m_length) && floatEqual(abox.m_height, bbox.m_height) &&
            floatLessThan(abox.m_width + bbox.m_width, bin_width))
        {
          if (!abox.equalsBox(bbox.getId()) && !hasCorrespondBoxes(abox, bbox) && !hasCorrespondBoxes(bbox, abox) &&
              !hasSimulatedBox(bbox.getId(), abox.getId()))
          {
            Box a(abox.m_length, abox.m_width + bbox.m_width, abox.m_height, abox.m_mass + bbox.m_mass,
        ...
        else if (floatEqual(abox.m_width, bbox.m_width) && floatEqual(abox.m_height, bbox.m_height) &&
                 floatLessEqual(abox.m_length + bbox.m_length, bin_length))
        {
          if (!abox.equalsBox(bbox.getId()) && !hasCorrespondBoxes(abox, bbox) && !hasCorrespondBoxes(bbox, abox) &&
              !hasSimulatedBox(bbox.getId(), abox.getId()))
          {
            Box a(abox.m_length + bbox.m_length, abox.m_width, abox.m_height, abox.m_mass + bbox.m_mass,
                  "Sim " + abox.m_name + "+" + bbox.m_name, abox.box_labels);
            a.is_simulated = true;
            a.m_id = highest_old_id + 1;
