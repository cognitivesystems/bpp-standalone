 ...
 */
  HoldingPlatform(double bin_l, double bin_w, bool GENERATE_SIMULATED_BOXES, std::vector<Box> boxes_on_holdingplatform,
                  int startid);
 ...
 *	to the rem list; False: Additionaly the boxes which have the box with Id i as corresponding box are added to the
 *list
 */
 ...
 */
   ...
   * \return A boolean value: True: A Simulated Box containing the box with Id i1 and a box with Id i2 already exists;
   * False: No such Simulated Box already exists
   */
