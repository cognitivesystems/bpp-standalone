...
void Bin::addNewBox(Box& new_box, FittingPoint fp, HoldingPlatform hold, double helt)
{
      ...
      case 1:
        switch (sim_box.arrangement)
        {
          ...
          case RIGHT:
            ...
            break;
          default:
            cout << "Error in add simulate boxes!";
        }
        break;
      case 2:
        switch (sim_box.arrangement)
  ...
  else
  {
    ...
    // check collision
    //        std::cout << "=======================================================box collised ? = " <<
    //        this->isColliding(new_box)<< std::endl;
...
Eigen::Vector3d Bin::checkNewBoxPosition(Box abox, FittingPoint fp)
{
  ...
  if (bulletPhysics->isPointContact(centerY))
  {
    distance(1) = 0.0;
  }
...
Eigen::Vector3d Bin::pushBoxTowardBinCenter(Box abox, FittingPoint fp, double helt)
{
  ...
  if (directionX(0) > 0)
  {
    position(0) = origin(0) + abox.m_length * directionX(0);
  }
  if (directionY(1) > 0)
  {
    position(1) = origin(1) + abox.m_width * directionY(1);
  }

  Eigen::Vector3d cp_0, cp_1, cp_2;
  cp_0 = position + abox.m_height * directionZ;
  cp_1 = position - abox.m_length * directionX;
  cp_1(2) += 0.1;
  cp_2 = position - abox.m_width * directionY;
  cp_2(2) += 0.1;
  ...
  if (floatLessThan(areaX, areaOld))
  {
    distance(0) = 0.0;
  }
  if (floatLessThan(areaY, areaOld))
  {
    distance(1) = 0.0;
  }
  ...
  Eigen::Vector3d centerX, centerY;
  centerX << position(0), position(1) - (1 / 2.0) * abox.m_width * directionY(1),
      position(2) + (1 / 2.0) * abox.m_height;

  centerY << position(0) - (1 / 2.0) * abox.m_length * directionX(0), position(1),
      position(2) + (1 / 2.0) * abox.m_height;
  ...
  if (fabs(distance(0)) > abox.m_length)
  {
    distance(0) = 0.0;
  }
  if (fabs(distance(1)) > abox.m_width)
  {
    distance(1) = 0.0;
  }

  // check4: if the box is near the center
  if (origin(0) < center(0) && (origin(0) + abox.m_length > center(0)))
  {
    distance(0) = 0.0;
  }
  if (origin(1) < center(1) && (origin(1) + abox.m_width > center(1)))
  {
    distance(1) = 0.0;
  }
  ...
  if (std::fabs(distance(0)) < 0.03)
  {
    distance(0) = 0.0;
  }
  if (std::fabs(distance(1)) < 0.03)
  {
    distance(1) = 0.0;
  }
  ...
  if (std::fabs(distance(0)) > 0 && std::fabs(distance(1)) > 0)
  {
    ...
    {
      distance(1) = 0.0;
      std::cout << "Move both x and y, collision!\n";
    }
  }

#ifdef DEBUG_PRINT
  std::cout << "--------------------------------------------------------------- box move distance "
