...
Box::Box(double length, double width, double height, double mass, std::string name, std::vector<std::string> labels,
         std::string material, std::string tool)
  : box_labels(labels), material(material), tool_name(tool)
{
  m_length = length;
  m_width = width;
  m_height = height;
  ...
  if (tool_name == "")
  {
    if ((mass < 50 || m_length < 0.75) && (material == "Carton" || material == "carton"))
    {
      ...
      z_max = 4.304 - 0.093 - 0.487;
             ...
             (material == "Styrofoam" || material == "styrofoam" || material == "Wooden" || material == "wooden"))
    {
