...
//
{
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      t = xm * log(f1 / x1) + (n - m + 0.5) * log(z / w) + (double)(ix - m) * log(w * p / (x1 * q)) +
...
//
{
  ...
  else
  {
    ...
    value = exp(temp) * pow(rval, alpha - 1.0) * pow(1.0 - rval, beta - 1.0);
  }
  return value;
}
//****************************************************************************80
...
//
{
  ...
  else
  {
    ...
    temp1 = (temp2 - 1.0) * log(rval) - 0.5 * rval - temp2 * log(2.0) - r8_gamma_log(temp2);
...
//
{
  ...
  double p4[8] = { 1.474502166059939948905062E+04, 2.426813369486704502836312E+06, 1.214755574045093227939592E+08,
                   2.663432449630976949898078E+09, 2.940378956634553899906876E+10, 1.702665737765398868392998E+11,
  ...
  double q4[8] = { 2.690530175870899333379843E+03, 6.393885654300092398984238E+05, 4.135599930241388052042842E+07,
                   1.120872109616147941376570E+09, 1.488613728678813811542398E+10, 1.016803586272438228077304E+11,
...
//
{
  ...
  {
    temp = alpha * log(beta) + (alpha - 1.0) * log(rval) - beta * rval - r8_gamma_log(alpha);
...
//
{
  ...
  else
  {
    ...
    temp1 = -temp2 * log(2.0) - (temp2 + 1.0) * log(rval) - 0.5 / rval - r8_gamma_log(temp2);
...
//
{
  ...
  else
  {
    ...
    temp1 = temp2 * log(temp2) + temp2 * log(s) - (temp2 * s / rval) - (temp2 + 1.0) * log(rval) - r8_gamma_log(temp2);
...
void r8ge_print_some(int m, int n, double a[], int ilo, int jlo, int ihi, int jhi, string title)
...
double r8vec_multinormal_pdf(int n, double mu[], double r[], double c_det, double x[])
...
//
{
  ...
  pdf = 1.0 / sqrt(pow(2.0 * pi, n)) * 1.0 / sqrt(c_det) * exp(-0.5 * xcx);
